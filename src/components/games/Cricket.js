...
const Cricket = () => {
  ...
  useEffect(() => {
    axios
      .get("http://13.201.93.229:5000/api/auth/getMatches_cricket")
  ...
  useEffect(() => {
    axios
      .get(`http://13.201.93.229:5000/api/auth/sportsItems/name/cricket`)
      .then((response) => {
  ...
  const handleUpdateScore = async (team) => {
    try {
      ...
      const response = await axios.put(`http://13.201.93.229:5000/api/auth/update-score_cricket/${matchId}`, {
        team,
        score: newScore,
      });
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      axios
        .put(
          `http://13.201.93.229:5000/api/auth/updateMatch_cricket/${selectedMatch._id}`,
          {
      ...
      // Add new match
      axios
        .post("http://13.201.93.229:5000/api/auth/addMatch_cricket", {
          teamA: newMatch.teamA,
          teamB: newMatch.teamB,
          name: `${newMatch.teamA || "Team A"} VS ${
            newMatch.teamB || "Team B"
  ...
  const handleDeleteMatch = () => {
    if (selectedMatch) {
      axios
        .delete(
          `http://13.201.93.229:5000/api/auth/deleteMatch_cricket/${selectedMatch._id}`
        )
        .then(() => {
          setMatches(
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(
        `http://13.201.93.229:5000/api/auth/getPlayers_cricket/${match._id}?team=TeamA`
      )
    ...
    axios
      .get(
        `http://13.201.93.229:5000/api/auth/getPlayers_cricket/${match._id}?team=TeamB`
      )
  ...
  const handleAddPlayer = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://13.201.93.229:5000/api/auth/addPlayers_cricket/${selectedMatch._id}`,
          {
            ...playerFormData,
  ...
  const handleUpdatePlayer = () => {
    if (selectedMatch && selectedPlayerDetails) {
      ...
      axios
        .put(
          `http://13.201.93.229:5000/api/auth/updatePlayerDetails_cricket/${matchId}/${playerId}`,
          {
  ...
  const handleDeletePlayer = () => {
    if (selectedPlayerDetails) {
      ...
      axios
        .delete(
          `http://13.201.93.229:5000/api/auth/deletePlayerDetails_cricket/${playerId}/${matchId}`
        )
        .then(() => {
          const updatedPlayers =
