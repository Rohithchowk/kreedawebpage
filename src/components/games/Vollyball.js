...
const Vollyball = () => {
  ...
  const [teamCScore, setTeamCScore] = useState('');
  const [teamDScore, setTeamDScore] = useState('');

  const [teamEScore, setTeamEScore] = useState('');
  const [teamFScore, setTeamFScore] = useState('');
  ...
  useEffect(() => {
    axios
      .get("http://13.201.93.229:5000/api/auth/getMatches_volly")
      .then((response) => setMatches(response.data))
      .catch((error) => console.error(error));

    axios
      .get(`http://13.201.93.229:5000/api/auth/sportsItems/name/vollyball`)
      .then((response) => {
      ...
      if (selectedMatch) {
        setTeamAScore(selectedMatch.scoreC[0]?.teamScore || '');
        setTeamBScore(selectedMatch.scoreD[0]?.teamScore || '');
      }
      if (selectedMatch) {
        setTeamEScore(selectedMatch.scoreE[0]?.teamScore || '');
        setTeamFScore(selectedMatch.scoreF[0]?.teamScore || '');
      }
  ...
  const handleUpdateScore = async (team) => {
    try {
      ...
      const response = await axios.put(`http://13.201.93.229:5000/api/auth/update-score_volly/${matchId}`, {
        team,
        score: newScore,
      });
  ...
  const handleUpdateScorenew = async (team) => {
    try {
      const matchId = selectedMatch._id;
      let newScore;
      if (team === 'C') {
      ...
      // Send update request to backend
      const response = await axios.put(`http://13.201.93.229:5000/api/auth/update-score_vollynew/${matchId}`, {
        team,
        score: newScore,
      });
  ...
  const handleUpdateScorenews = async (team) => {
    try {
      const matchId = selectedMatch._id;
      let newScore;
      if (team === 'E') {
      ...
      // Send update request to backend
      const response = await axios.put(`http://13.201.93.229:5000/api/auth/update-score_vollynews/${matchId}`, {
        team,
        score: newScore,
      });
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      axios
        .put(
          `http://13.201.93.229:5000/api/auth/updateMatch_volly/${selectedMatch._id}`,
          {
      ...
      // Add new match
      axios
        .post("http://13.201.93.229:5000/api/auth/addMatch_volly", {
          teamA: newMatch.teamA,
          teamB: newMatch.teamB,
          name: `${newMatch.teamA || "Team A"} VS ${
            newMatch.teamB || "Team B"
  ...
  const handleDeleteMatch = () => {
    if (selectedMatch) {
      axios
        .delete(
          `http://13.201.93.229:5000/api/auth/deleteMatch_volly/${selectedMatch._id}`
        )
        .then(() => {
          setMatches(
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(
        `http://13.201.93.229:5000/api/auth/getPlayers_volly/${match._id}?team=TeamA`
      )
    ...
    axios
      .get(
        `http://13.201.93.229:5000/api/auth/getPlayers_volly/${match._id}?team=TeamB`
      )
  ...
  const handleAddPlayer = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://13.201.93.229:5000/api/auth/addPlayers_volly/${selectedMatch._id}`,
          {
            ...playerFormData,
  ...
  const handleUpdatePlayer = () => {
    if (selectedMatch && selectedPlayerDetails) {
      ...
      axios
        .put(
          `http://13.201.93.229:5000/api/auth/updatePlayerDetails_volly/${matchId}/${playerId}`,
          {
  ...
  const handleDeletePlayer = () => {
    if (selectedPlayerDetails) {
      ...
      axios
        .delete(
          `http://13.201.93.229:5000/api/auth/deletePlayerDetails_volly/${playerId}/${matchId}`
        )
        .then(() => {
          const updatedPlayers =
...
<tr>
      ...
      <td>{selectedMatch.scoreC[0]?.teamCScore}</td>
      {isAdmin && selectedMatch && (
        <td>
        <input
          type="number"
          value={teamCScore}
          onChange={(e) => setTeamCScore(e.target.value)}
        />
        <button onClick={() => handleUpdateScorenew('C')}>Update Score</button>
      </td>

      )}
      <td>{selectedMatch.scoreE[0]?.teamEScore}</td>
      {isAdmin && selectedMatch && (
        <td>
        <input
          type="number"
          value={teamEScore}
          onChange={(e) => setTeamEScore(e.target.value)}
        />
        <button onClick={() => handleUpdateScorenews('E')}>Update Score</button>
      </td>
...
<td>{selectedMatch.scoreD[0]?.teamDScore}</td>
      {isAdmin && selectedMatch && (
        <td>
        <input
          type="number"
          value={teamDScore}
          onChange={(e) => setTeamDScore(e.target.value)}
        />
        <button onClick={() => handleUpdateScorenew('D')}>Update Score</button>
      </td>
      )}

<td>{selectedMatch.scoreF[0]?.teamFScore}</td>
      {isAdmin && selectedMatch && (
        <td>
        <input
          type="number"
          value={teamFScore}
          onChange={(e) => setTeamFScore(e.target.value)}
        />
        <button onClick={() => handleUpdateScorenews('F')}>Update Score</button>
      </td>
      )}
    </tr>
              </>
